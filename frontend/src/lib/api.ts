import {
  APIResponse,
  Application,
  DashboardStats,
  Interview,
  Notification,
  PaginatedResponse,
  User,
} from '@/types';

const API_BASE_URL =
  process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000/api';

async function request<T>(
  endpoint: string,
  options: RequestInit = {}
): Promise<T> {
  const token = localStorage.getItem('applytrack_token');

  const headers = {
    'Content-Type': 'application/json',
    ...(token ? { Authorization: `Bearer ${token}` } : {}),
    ...options.headers,
  };

  const response = await fetch(`${API_BASE_URL}${endpoint}`, {
    ...options,
    headers,
  });

  const result = await response.json();

  if (!response.ok) {
    throw new Error(result.message || 'API request failed');
  }

  return result as T;
}

export const api = {
  auth: {
    register: (data: any) =>
      request<APIResponse<User>>('/auth/register', {
        method: 'POST',
        body: JSON.stringify(data),
      }),

    login: (data: any) =>
      request<APIResponse<{ token: string; user: User }>>('/auth/login', {
        method: 'POST',
        body: JSON.stringify(data),
      }),

    me: () => request<APIResponse<User>>('/auth/me'),
  },

  applications: {
    getAll: (params: any) => {
      const query = new URLSearchParams(params).toString();
      return request<PaginatedResponse<Application>>(
        `/applications?${query}`
      );
    },

    getOne: (id: string) =>
      request<APIResponse<Application>>(`/applications/${id}`),

    create: (data: any) =>
      request<APIResponse<Application>>('/applications', {
        method: 'POST',
        body: JSON.stringify(data),
      }),

    update: (id: string, data: any) =>
      request<APIResponse<Application>>(`/applications/${id}`, {
        method: 'PUT',
        body: JSON.stringify(data),
      }),

    delete: (id: string) =>
      request<APIResponse<null>>(`/applications/${id}`, {
        method: 'DELETE',
      }),

    updateStatus: (id: string, status: string) =>
      request<APIResponse<Application>>(`/applications/${id}/status`, {
        method: 'PATCH',
        body: JSON.stringify({ status }),
      }),
  },

  interviews: {
    getAll: () =>
      request<APIResponse<Interview[]>>('/interviews'),

    getOne: (id: string) =>
      request<APIResponse<Interview>>(`/interviews/${id}`),

    create: (data: any) =>
      request<APIResponse<Interview>>('/interviews', {
        method: 'POST',
        body: JSON.stringify(data),
      }),

    update: (id: string, data: any) =>
      request<APIResponse<Interview>>(`/interviews/${id}`, {
        method: 'PUT',
        body: JSON.stringify(data),
      }),

    delete: (id: string) =>
      request<APIResponse<null>>(`/interviews/${id}`, {
        method: 'DELETE',
      }),
  },

  notifications: {
    getAll: (params: any) => {
      const query = new URLSearchParams(params).toString();
      return request<APIResponse<Notification[]>>(
        `/notifications?${query}`
      );
    },

    getCount: () =>
      request<APIResponse<{ count: number }>>('/notifications/count'),

    markRead: (id: string) =>
      request<APIResponse<Notification>>(`/notifications/${id}/read`, {
        method: 'PATCH',
      }),

    markAllRead: () =>
      request<APIResponse<null>>('/notifications/read-all', {
        method: 'PATCH',
      }),

    delete: (id: string) =>
      request<APIResponse<null>>(`/notifications/${id}`, {
        method: 'DELETE',
      }),
  },

  dashboard: {
    getStats: () =>
      request<APIResponse<DashboardStats>>('/dashboard/stats'),

    getTrends: () =>
      request<APIResponse<any>>('/dashboard/trends'),
  },
};